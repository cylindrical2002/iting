...
public class GQLByteStringLiteralTest {
    @TestFactory
    public Stream<DynamicTest> generateByteStringLiteralTests() throws Exception {
        URL resource = getClass().getResource("/byte_string_literal.txt");
        if (resource == null) {
            ...
            if (!section.trim().isEmpty()) {
                ...
                tests.add(DynamicTest.dynamicTest("Test " + testIndex, () -> {
                    ...
                    boolean correctSeparator = tokens.getTokens().size() == 2
                            && tokens.getTokens().get(0).getType() == GQLLexer.BYTE_STRING_LITERAL
